		...
		i.forEach((e) => {
			...
			Array.isArray(e.subtasks) && e.subtasks.forEach((r) => {
				if (r.intervalHours) {
					...
					let o = new Date(a.getTime() + r.intervalHours * 60 * 60 * 1e3);
					if (o <= l) {
						let i = `recurring-interval:${r.id}:${a.getTime()}`;
						if (!t.has(i)) {
							if (t.add(i), l.getTime() - o.getTime() <= 12e4 && n.isSupported()) {
								let t = new n({
		...
		n.forEach((e) => {
			...
			Array.isArray(e.subtasks) && e.subtasks.forEach((e) => {
				if (e.intervalHours) {
					...
					let t = null;
					a && typeof a == "object" && Object.entries(a).forEach(([n, r]) => {
						Array.isArray(r) && r.forEach((r) => {
							let i = typeof r == "string" ? r : r.subtaskId;
							if (i === e.id) {
								let e = typeof r == "string" ? (/* @__PURE__ */ new Date(n + "T12:00:00")).toISOString() : r.timestamp;
								(!t || e > t.timestamp) && (t = {
									subtaskId: i,
									timestamp: e
								});
					...
					let n = t ? new Date(t.timestamp) : c;
					if (!t) {
						a[l] || (a[l] = []), a[l].push({
							subtaskId: e.id,
					...
					new Date(n.getTime() + e.intervalHours * 60 * 60 * 1e3) <= c && (a[l] || (a[l] = []), a[l].push({
						subtaskId: e.id,
						timestamp: c.toISOString()
					}), u = !0);
				} else if (e.time) {
					if (e.days && e.days.length > 0) {
						...
						if (!e.days.includes(t)) return;
					}
					if (!(a?.[l] || []).some((t) => (typeof t == "string" ? t : t.subtaskId) === e.id)) {
