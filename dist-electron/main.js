...
function createTray() {
	...
	try {
		...
		if (fs.existsSync(iconPath)) icon = nativeImage.createFromPath(iconPath).resize({
			width: 16,
			height: 16
		});
		else icon = nativeImage.createFromDataURL(defaultIconBase64);
	} catch (error) {
...
function createWindow() {
	...
	win = new BrowserWindow({
		...
		icon: fs.existsSync(winIconPath) ? winIconPath : void 0,
		webPreferences: {
		...
		if (Array.isArray(recurringGroups)) recurringGroups.forEach((group) => {
			if (Array.isArray(group.subtasks)) group.subtasks.forEach((subtask) => {
				if (subtask.intervalHours) {
					...
					const nextDue = new Date(lastTime.getTime() + subtask.intervalHours * 60 * 60 * 1e3);
					if (nextDue <= now) {
						const key = `recurring-interval:${subtask.id}:${lastTime.getTime()}`;
						if (!notifiedKeys.has(key)) {
							notifiedKeys.add(key);
							if (now.getTime() - nextDue.getTime() <= 12e4 && Notification.isSupported()) {
								const notification = new Notification({
